...
class PathAutocomplete {
    provideCompletionItems(document, position, token) {
        ...
        var folderItems = this.getFolderItems(foldersPath).then((items) => {
    ...
    getFolderItems(foldersPath) {
        var results = foldersPath.map(folderPath => {
            return new Promise(function (resolve, reject) {
                fs.readdir(folderPath, function (err, items) {
                    ...
                    items.forEach(item => {
                        try {
                            fileResults.push(new FileInfo_1.FileInfo(path.join(folderPath, item)));
                        }
                        catch (err) {
        ...
        return Promise.all(results).then(allResults => {
            return allResults.reduce((all, currentResults) => {
