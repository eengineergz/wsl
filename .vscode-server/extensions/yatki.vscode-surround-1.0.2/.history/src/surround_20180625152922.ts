...
function getSurroundConfig(): ISurroundConfig {
  ...
  const allConfig: ISurroundConfig = {};
  const withConfig: ISurroundConfig = config.get("with", {});
  const customConfig: ISurroundConfig = config.get("custom", {});
  Object.keys(withConfig).forEach(configKey => {
    allConfig[configKey] = withConfig[configKey];
  });

  Object.keys(customConfig).forEach(configKey => {
    allConfig[configKey] = customConfig[configKey];
  });
