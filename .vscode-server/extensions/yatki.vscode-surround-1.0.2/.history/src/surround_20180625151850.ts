...
function getSurroundConfig(): ISurroundConfig {
  ...
  const items = <object>config.get("with", {});
  const custom = <object>config.get("custom", {});
  Object.keys(custom).forEach(customItemKey => {
    const surroundItem: ISurroundItem = custom[customItemKey];
    if (!surroundItem.disabled) {
      items.push(surroundItem);
    }
  });

  return <ISurroundConfig>items;
}
